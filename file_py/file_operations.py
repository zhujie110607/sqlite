...
class FileManager:
    ...
    def add(self):
        if QMessageBox.warning(None, "系统消息", "您点击了添加按钮，请确认是否要添加数据") == QMessageBox.Yes:
    ...
    def update(self):
        if QMessageBox.warning(None, "系统消息", "您点击了修改按钮，请确认是否要修改数据") == QMessageBox.Yes:
            path = select_file("请选择要添加数据的Excel文件")
            if path:
    ...
    def delete(self):
        if QMessageBox.warning(None, "系统消息", "您点击了删除按钮，请确认是否要删除数据") == QMessageBox.Yes:
            path = select_file("请选择要添加数据的Excel文件")
            if path:
    ...
    def export(self):
        ...
        if QMessageBox.question(None, "系统消息", "是否导出数据") == QMessageBox.Yes:
            df = self.query_all()
            if df.empty:
